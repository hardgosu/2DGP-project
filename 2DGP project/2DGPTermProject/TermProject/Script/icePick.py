...
class IcePick(ObjectBase):
    ...
    def __init__(self,x,y,dir,damage):
        ...
        self.imageState = IcePick.explosion1
        ...
        self.row = IcePick.Images[self.imageState]["Row"] - 1
        self.check = False
    ...
    def update(self):
        ...
        elif int(self.frame) % IcePick.Images[self.imageState]["Column"]  == 0:
            if(not self.check):
                self.row -= 1
                self.check = True
        else:
        ...
        if(int(self.frame) >= IcePick.Images[self.imageState]["Frames"] - 1):
            self.destroy()
        ...
        self.frame = (self.frame + FRAMES_PER_ACTION * ACTION_PER_TIME * game_framework.frame_time) % IcePick.Images[self.imageState]["Frames"]
        #self.x += self.velocity * self.dir * game_framework.frame_time
    ...
    def draw(self):
        ...
        if self.boundingBoxOn:
            self.draw_bb()
        if self.dir == 1:
            IcePick.Images[self.imageState]["ImageFile"].clip_composite_draw((int(self.frame) % IcePick.Images[self.imageState]["Column"]) * IcePick.Images[self.imageState]["IntervalX"] + IcePick.Images[self.imageState]["XRevision"], IcePick.Images[self.imageState]["IntervalY"] * self.row, IcePick.Images[self.imageState]["IntervalX"], IcePick.Images[self.imageState]["IntervalY"], 0, '', self.x - self.curState.GetBackground().windowLeft, self.y - self.curState.GetBackground().windowBottom, IcePick.Images[self.imageState]["IntervalX"], IcePick.Images[self.imageState]["IntervalY"])
        else:
            IcePick.Images[self.imageState]["ImageFile"].clip_composite_draw((int(self.frame) % IcePick.Images[self.imageState]["Column"]) * IcePick.Images[self.imageState]["IntervalX"] + IcePick.Images[self.imageState]["XRevision"], IcePick.Images[self.imageState]["IntervalY"] * self.row, IcePick.Images[self.imageState]["IntervalX"], IcePick.Images[self.imageState]["IntervalY"], 0, 'h', self.x - self.curState.GetBackground().windowLeft, self.y - self.curState.GetBackground().windowBottom, IcePick.Images[self.imageState]["IntervalX"], IcePick.Images[self.imageState]["IntervalY"])
