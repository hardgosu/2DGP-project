...
class Background2:
    ...
    def __init__(self):
        ...
        self.imageState = Background2.idle
    ...
    def update(self):
        ...
        self.windowLeft = clamp(0,
                                ...
                                ((Background2.Images[self.imageState]["IntervalX"] - Background2.Images[self.imageState]["XRevision"]) * self.scaleX - self.curState.screenX) // self.scaleX)

                                #218)
        self.windowBottom = clamp(0,
                                  (int(self.centerObject.y) - self.canvasHeight // 2) // self.scaleY,
                                  #0)
                                  ((Background2.Images[self.imageState]["IntervalY"] - Background2.Images[self.imageState]["YRevision"]) * self.scaleY - self.curState.screenY) // self.scaleY)

        #self.set_direction()

        self.frame = (self.frame + FRAMES_PER_ACTION * ACTION_PER_TIME * game_framework.frame_time) % Background2.Images[self.imageState]["Frames"]
    ...
    def draw(self):
        ...
        Background2.Images[self.imageState]["ImageFile"].clip_draw_to_origin(self.windowLeft + int(self.frame) * Background2.Images[self.imageState]["IntervalX"], self.windowBottom, Background2.Images[self.imageState]["IntervalX"], Background2.Images[self.imageState]["IntervalY"] - (Background2.Images[self.imageState]["YRevision"] // self.scaleY), 0, 0, Background2.Images[self.imageState]["IntervalX"] * self.scaleX, Background2.Images[self.imageState]["IntervalY"] * self.scaleY)
